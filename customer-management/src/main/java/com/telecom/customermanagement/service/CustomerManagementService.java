                // Cross-service: If inventory cost allocation is enabled, allocate cost
                if (!FeatureFlagReader.isFeatureEnabled("inventory_enable_cost_allocation")) {
                    throw new IllegalStateException("Inventory cost allocation is disabled (inventory_enable_cost_allocation)");
                }
                // ...existing code...
        // Cross-service: If AI pricing is enabled in product-catalog, apply AI pricing logic
        if (FeatureFlagReader.isFeatureEnabled("catalog_enable_ai_pricing")) {
            return 42.0; // Example: AI-determined price
        }
        // ...existing code...
        return 100.0;
...
public class CustomerManagementService {
    ...
    public TelecomCustomer suspendCustomer(String customerId, String reason) {
        if (!FeatureFlagReader.isFeatureEnabled(FeatureFlagConstants.ENABLE_CUSTOMER_SUSPENSION)) {
            throw new RuntimeException("Customer suspension feature is disabled");
        }
